...
# try:
#     from env.settings import api_token
# except Exception:
#     logging.error("api_token not defined in env/settings.py, you should created it")

# try:
#     from env.settings import gitlab_api_url
# except Exception:
#     logging.error("gitlab_api_url not defined in env/settings.py, you should created it")

class GitlabConnector(object):
    server_users = {}
    server_users_emails = {}

    def __init__(self, gitlab_api_url, gitlab_user_token): 
        self.api_token = gitlab_user_token
        self.api_host = gitlab_api_url
    ...
    def get_server_users(self):
        ...
        while more_users:
            page += 1
            url = '{0}/v4/users/?page={1}&per_page=100'.format(self.api_host, page)
            logging.debug(url)
    ...
    def get_labels(self, project_id):
        url = '{0}/v4/projects/{1}/labels/?per_page=100'.format(self.api_host, project_id)
        response = requests.get(url, headers={'PRIVATE-TOKEN': self.api_token})
        return response.json()

    def get_timestamp(self, project_id, issue_iid):
        url = '{0}/v4/projects/{1}/issues/{2}/time_stats'.format(self.api_host, project_id, issue_iid)
        logging.debug(url)
        response = requests.get(url, headers={'PRIVATE-TOKEN': self.api_token})
        return response.json()

    def get_comments(self, project_id, issue_iid):
        url = '{0}/v4/projects/{1}/issues/{2}/notes?per_page=100'.format(self.api_host, project_id, issue_iid)
        logging.debug(url)
        response = requests.get(url, headers={'PRIVATE-TOKEN': self.api_token})
        comments = []
    ...
    def get_issues(self, project_id=None):
        ...
        while more:
            page += 1
            if project_id:
                url = '{0}/v4/projects/{1}/issues?state=opened&page={2}&per_page=100'.format(self.api_host, project_id, page)
                logging.debug(url)
                response = requests.get(url, headers={'PRIVATE-TOKEN': self.api_token})
                more = response.json()
                all_issues += more
